...
def time2rank(time_img: np.ndarray) -> np.ndarray:
    ...
    rank = np.zeros(time_img.shape, dtype=np.float32)
...
def process_id(
    ...
    train_folder: os.PathLike,
    test_cids: list,
    brain_out_folder: os.PathLike,
    test_folder: os.PathLike,
    label_test_folder: os.PathLike,
    brain_out_test_folder: os.PathLike,
    ...
    # Brain file
    brain_file = (
        os.path.join(brain_folder, f"{i}_brain.nii.gz")
...
def main(args):
    ...
    assert os.path.exists(
        dist_folder
    ), f"Distance folder '{dist_folder}' does not exist"
    ...
    if brain_folder is not None:
        assert os.path.exists(
            brain_folder
        ), f"Brain folder '{brain_folder}' does not exist"
        # Create output brain folder
        brain_out_folder = os.path.join(
            out_folder, "raw_cropped", task_folder, "brainTr"
        )
        brain_out_test_folder = os.path.join(
            out_folder, "raw_cropped", task_folder, "brainTs"
        )
    ...
    # Obtain CIDs
    cta_cids = np.array(
        [f.replace(".nii.gz", "") for f in sorted(os.listdir(cta_folder))], dtype=str
    )
    dist_cids = np.array(
        [f.replace(".nii.gz", "") for f in sorted(os.listdir(dist_folder))], dtype=str
    )
    time_cids = np.array(
        [f.replace(".nii.gz", "") for f in sorted(os.listdir(time_folder))], dtype=str
    )
    ...
    np.random.seed(42)
    test_cids = np.random.choice(common_cids, size=sample_size, replace=False)
...
def get_args():
    # Prepare training data for CTA-to-time-vessel map generator
