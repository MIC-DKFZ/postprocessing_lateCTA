...
from registration.registration_utils import (
...
def register_frame(
    cta_image: sitk.Image,
    ctp_file: os.PathLike,
    ...
    """
    assert "default_val" in list(
        cfg.keys()
    ), "Key 'default_val' absent in registration configuration"
    ...
    try:
        transform_matrix = get_transformation_matrix(
            fixed=cta_image, moving=ctp_image, clipvalue=[None, None], parameters=p
        )
    except:
        logger.info("There was an error, registering first CTA to CTP...")

        transform_matrix = get_transformation_matrix(
            fixed=ctp_image, moving=cta_image, clipvalue=[None, None], parameters=p
        )

        new_cta = apply_transformation(
            transform_matrix,
            moving=cta_image,
            segmentation=False,
        ...
        transform_matrix = get_transformation_matrix(
            fixed=new_cta, moving=ctp_image, clipvalue=[None, None], parameters=p
        )
...
def check_suitability(
    cta_file: os.PathLike, ctp_folder: os.PathLike, outfile: os.PathLike
):
    """
    ...
    if os.path.exists(in_folder):
        ...
        if (overwrite.lower().strip() == "n") and (os.path.exists(out_folder)):
            ...
            if not (np.array_equal(input_ctp_files, output_ctp_files)):
                ...
                files_register = [
                    os.path.join(in_folder, remaining_file)
...
def execute_registration(
    cta_file: os.PathLike,
    ctp_folder: os.PathLike,
    out_folder: os.PathLike,
    cfg: dict,
    p: dict,
):
...
def main(args):
    ...
    assert os.path.exists(ctp_folder) and os.path.exists(
        cta_folder
    ), f"Folder with CTA data '{cta_folder}' or with CTP data '{ctp_folder}' does not exist"
    assert os.path.exists(
        os.path.dirname(out_folder)
    ), f"Parent folder of output folder '{out_folder}' does not exist"
    ...
    p = set_parameters(
        method=cfg["method"], DefaultPixelValue=cfg["default_val"], metric=cfg["metric"]
    )
        ...
        if ".nii.gz" in cta_file:
            ...
            if not (os.path.exists(outfile)):
                ...
                if os.path.exists(ctp_subfolder):
                    check_suitability(
                        cta_file=full_cta_file, ctp_folder=out_folder, outfile=outfile
                    )
