...
def binarize_variation_image(cv_img: np.ndarray, cfg: dict) -> np.ndarray:
    ...
    assert (
        ("n_clusters" in list(cfg.keys()))
        and ("state" in list(cfg.keys()))
        and ("opening_iters" in list(cfg.keys()))
        and ("remove_thr" in list(cfg.keys()))
    ), "Keys 'n_clusters' and/or 'state' and/or 'opening_iters' and/or 'remove_thr' not in configuration"

    assert (
        cfg["n_clusters"] > 1
    ), "One or less clusters designated for variation image creation"

    # Apply k-means thresholding to isolate high variation information
    model = KMeans(n_clusters=cfg["n_clusters"], random_state=cfg["state"])
    ...
    if cfg["remove_thr"] > 0:
        bin_img = remove_small_objects(
            bin_img.astype(bool), min_size=cfg["remove_thr"]
        ).astype(float)
...
def trim_ctp(ctp_array: np.ndarray, cfg: dict) -> Union[np.ndarray, int, int]:
    ...
    """
    assert "ctp_trimming" in list(
        cfg.keys()
    ), "'ctp_trimming' not in configuration keys"
    ...
    for i in range(ctp_array.shape[1]):
        ...
        if not (all_equal):
            ...
            var = np.abs(
                mask_ctp_array[:, i] - mask_ctp_array[:, (low_slice + high_slice) // 2]
            ).sum() / (
                mask_ctp_array[:, (low_slice + high_slice) // 2].sum()
                + np.finfo(float).eps
            )
...
def segment_brain_components(
    cta_folder: os.PathLike, segm_folder: os.PathLike, image, cid: str
    ...
    if (
        not (os.path.exists(skull_file))
        or not (os.path.exists(brain_file))
        or not (os.path.exists(cca_file))
        or not (os.path.exists(ica_file))
    ):
...
def derive_tta(ctp_array: np.ndarray, time_array: np.ndarray) -> np.ndarray:
    ...
    resolution_array = (
        time_array[time_array.shape[0] // 2]
        - time_array[time_array.shape[0] // 2].min()
    )
...
def detect_slices_artifacts(
    img: np.ndarray, low_slice: int, high_slice: int
) -> Union[float, float, np.ndarray]:
    """
    ...
    if (
        high_slice_new - low_slice_new == 2
    ):  # Too much trimming, almost consecutive extreme fragments
...
def separate_arterial_venous(
    tta_img: np.ndarray, cfg: dict, image, out: os.PathLike, cid: str
):
    """
    ...
    """
    assert "separation_method" in list(
        cfg.keys()
    ), "'separation_method' key not in configuration"
    ...
    if method.lower().strip() == "percentile":
        ...
        assert "n_clusters_separation" in list(
            cfg.keys()
        ) and "state_separation" in list(
            cfg.keys()
        ), "'n_clusters_separation' and/or 'state_separation' keys not in configuration"
        assert (
            cfg["n_clusters_separation"] > 1
        ), "One or less clusters designated for artery-vein separation"
        # K means model
        model_time = KMeans(
            n_clusters=cfg["n_clusters_separation"],
        ...
        raise ValueError(
            "Wrong artery-vein separation method. Choose between the following methods: 'percentile' for percentile-based separation, or 'kmeans' for k-means based separation"
        )
...
def case_analysis(
    folder: os.PathLike,
    time_folder: os.PathLike,
    skull_folder: os.PathLike,
    cta_folder: os.PathLike,
    out: os.PathLike,
    out_cta: os.PathLike,
    cid: str,
    cfg: dict,
    ...
    assert "segment_skull_brain" in list(
        cfg.keys()
    ), "Key 'segment_skull_brain' not in configuration keys"
    ...
    if do_segm_skull_brain == 1:
        ...
        brain_segm, skull_segm, cca_segm, ica_segm = segment_brain_components(
            cta_folder=cta_folder, segm_folder=skull_folder, image=image, cid=cid
        )
    ...
    if do_segm_skull_brain:
        ...
        assert "erode_brain_iters" in list(
            cfg.keys()
        ), "Key 'erode_brain_iters' not in configuration"
        erode_brain_iters = cfg["erode_brain_iters"]
        if erode_brain_iters > 0:
            brain_segm = binary_erosion(
                brain_segm, iterations=erode_brain_iters
            ).astype(np.int32)
    ...
    low_slice, high_slice, variation_img = detect_slices_artifacts(
        img=variation_img, low_slice=low_slice, high_slice=high_slice
    )
    ...
    ), f"Median filter size '{size}' is not integer or negative"
    ...
    if do_separation.lower().strip() == "y":
        ...
        separate_arterial_venous(
            tta_img=tta_img, cfg=cfg, image=tta_image, out=out, cid=cid
        )
...
def main(args):
    ...
    assert os.path.exists(
        ctp_folder
    ), f"Preprocessed CTP folder '{ctp_folder}' does not exist"
    assert os.path.exists(
        cta_folder
    ), f"Preprocessed CTA folder '{cta_folder}' does not exist"
    assert os.path.exists(
        time_folder
    ), f"Preprocessed time folder '{time_folder}' does not exist"
    assert os.path.exists(
        os.path.dirname(out_cta_folder)
    ), f"Parent folder of masked CTA folder '{out_cta_folder}' does not exist"
    assert os.path.exists(
        os.path.dirname(skull_folder)
    ), f"Parent folder of skull segmentation folder '{skull_folder}' does not exist"
    assert os.path.exists(
        os.path.dirname(out)
    ...
    for cid in cids:
        ...
        if not (os.path.exists(outfile)) or cfg["overwrite"].lower().strip() == "y":
            ...
            case_analysis(
                ...
                skull_folder=skull_folder,
                cta_folder=cta_folder,
                out=out,
                out_cta=out_cta_folder,
                cid=cid,
                cfg=cfg,
            )
